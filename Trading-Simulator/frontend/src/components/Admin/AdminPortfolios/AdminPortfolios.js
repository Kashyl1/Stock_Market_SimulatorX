...
const AdminPortfolios = ({ userId }) => {
  ...
  const handleEditPortfolio = async (portfolioId) => {
    ...
    try {
      ...
      notyf.success("Portfolio name updated successfully!");
    } catch (error) {
      console.error('Error updating portfolio:', error);
      notyf.error("Failed to update portfolio name.");
    } finally {
      setTimeout(() => setEditStatus(null), 3000);
    }
  };
...
const handleDeletePortfolio = async (portfolioId) => {
  try {
    ...
    setAllPortfolios((prevPortfolios) => prevPortfolios.filter((p) => p.portfolioid !== portfolioId));
    setFilteredPortfolios((prevPortfolios) => prevPortfolios.filter((p) => p.portfolioid !== portfolioId));
    setTotalPages(Math.ceil((allPortfolios.length - 1) / pageSize));
    notyf.success("Portfolio was deleted successfully!");
  } catch (error) {
    notyf.error("Error deleting portfolio:");
  }
};
      ...
      {selectedPortfolio ? (
        ...
        <div>
          ...
          <div className="portfolio-grid">
            {filteredPortfolios
              .filter((portfolio) => !portfolio.deleted)
              .map((portfolio) => (
